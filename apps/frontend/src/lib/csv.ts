...
export interface ParsedLeads {
  emails: string[];
  /** Addresses found before de-duplication, for reporting what was skipped. */
  totalFound: number;
  duplicates: number;
}

export function extractEmails(content: string): ParsedLeads {
  const matches = content.match(EMAIL_REGEX) ?? [];

  const seen = new Set<string>();
  for (const match of matches) seen.add(match.trim().toLowerCase());
